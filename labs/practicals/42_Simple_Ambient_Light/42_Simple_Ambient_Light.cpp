...
bool render() {
  ...
  for (auto &e : meshes) {
	...
	glUniform4fv(eff.get_uniform_location("material colour"), 1, value_ptr(vec4(1.0f, 0.0f, 0.0f, 1.0f)));
    // Set ambient intensity - (0.3, 0.3, 0.3, 1.0)
	glUniform4fv(eff.get_uniform_location("ambient intensity"), 1, value_ptr(vec4(0.3f, 0.3f, 0.3f, 1.0f)));
    // *********************************
    // Render mesh
    renderer::render(m);
  }
